...
function GenericGenerator(; λ::AbstractFloat=0.1, kwargs...)
    return Generator(; penalty=default_distance, λ=λ, kwargs...)
end

"Constructor for `WachterGenerator`."
function WachterGenerator(; λ::AbstractFloat=0.1, kwargs...)
    return Generator(; penalty=Objectives.distance_mad, λ=λ, kwargs...)
end
...
function REVISEGenerator(; λ::AbstractFloat=0.1, latent_space=true, kwargs...)
    return Generator(; penalty=default_distance, λ=λ, latent_space=latent_space, kwargs...)
end
...
function GreedyGenerator(; η=0.1, n=nothing, kwargs...)
    ...
    return Generator(; penalty=default_distance, λ=0.0, opt=opt, kwargs...)
end

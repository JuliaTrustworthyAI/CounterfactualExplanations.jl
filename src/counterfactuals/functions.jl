...
function CounterfactualExplanation(;
    ...
    params = Dict(
        ...
        :T => T,
        :mutability =>
            repeat(DataPreprocessing.mutability_constraints(data), outer = size_),
        :initial_mutability =>
            repeat(DataPreprocessing.mutability_constraints(data), outer = size_),
    )
    ...
    ids =
        getindex.(findall(data.y .== counterfactual_explanation.target_encoded[:, :, 1]), 2)
    n_candidates = minimum([size(data.y, 2), 1000])
    candidates = select_factual(data, rand(ids, n_candidates))
    counterfactual_explanation.params[:potential_neighbours] =
        reduce(hcat, map(x -> x[1], collect(candidates)))
    ...
    counterfactual_explanation.search = Dict(
        ...
        :times_changed_features =>
            zeros(size(counterfactual_explanation.f(counterfactual_explanation.s′))),
    ...
    if counterfactual_explanation.latent_space &&
       !counterfactual_explanation.search[:terminated]
        ...
        counterfactual_explanation.s′, _, _ =
            GenerativeModels.rand(generative_model.encoder, counterfactual_explanation.s′)

        # NOTE! This is not very clean, will be improved.
        if generative_model.params.nll == Flux.Losses.logitbinarycrossentropy
            counterfactual_explanation.f = function (s)
                Flux.σ.(generative_model.decoder(s))
            end
        else
            counterfactual_explanation.f = function (s)
                generative_model.decoder(s)
            end
        end
...
"""
counterfactual(counterfactual_explanation::CounterfactualExplanation) =
    counterfactual_explanation.f(counterfactual_explanation.s′)
...
function update!(counterfactual_explanation::CounterfactualExplanation)
    ...
    _times_changed = reshape(
        counterfactual_explanation.f(Δs′) .!= 0,

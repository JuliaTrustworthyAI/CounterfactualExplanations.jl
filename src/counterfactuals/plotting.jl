...
function plot(
    ...
    T = isnothing(plot_up_to) ? total_steps(counterfactual_explanation) : minimum([plot_up_to, T])
    T += 1
    ingredients = set_up_plots(counterfactual_explanation; alpha=alpha_, plot_proba=plot_proba, kwargs...)
    ...
    plt = plot_proba ? plot(ingredients.p1,ingredients.p2;kwargs...) : plot(ingredients.p1;kwargs...)
...
function animate_path(
    counterfactual_explanation::CounterfactualExplanation, path=tempdir(); 
    alpha_=0.5,plot_up_to::Union{Nothing,Int}=nothing, plot_proba::Bool=false, kwargs...
)
    T = total_steps(counterfactual_explanation)
    T = isnothing(plot_up_to) ? total_steps(counterfactual_explanation) : minimum([plot_up_to, T])
    ...
    anim = @animate for t ∈ 1:T
        final_state = t == T
        plot_state(counterfactual_explanation, t, final_state; ingredients...)
        plot_proba ? plot(ingredients.p1,ingredients.p2;kwargs...) : plot(ingredients.p1;kwargs...)
    end
    return anim
end
...
function plot_state(
    counterfactual_explanation::CounterfactualExplanation, t::Int, final_sate::Bool;
    kwargs...
) 
    ...
    y = Int.(vec(args.path_labels[t]))
    n_ = counterfactual_explanation.num_counterfactuals
    label_ = reshape(["C$i" for i in 1:n_],1,n_)
    if !final_sate
        scatter!(args.p1,x1,x2,color=y;ms=5,label="")
    else
        scatter!(args.p1,x1,x2,color=y;ms=10,label="")
        if n_ > 1
...
function set_up_plots(counterfactual_explanation::CounterfactualExplanation; alpha, plot_proba, kwargs...)
    p1 = Models.plot(
        counterfactual_explanation.M, counterfactual_explanation.data; 
        target=counterfactual_explanation.target, alpha=alpha,
        kwargs...
    )

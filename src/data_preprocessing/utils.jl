...
- `keep_class_ratio=false`: Decides whether sample equally from each class, or keep their relative size.
...
function train_test_split(data::CounterfactualData; test_size=0.2, keep_class_ratio=false)
    ...
    if keep_class_ratio
        class_ratios = [
                            length(findall(vec(y .== cls))) / length(y)
                            for cls in classes_
                        ]
        class_samples = [
                            sample(
                                findall(vec(y .== cls)),
                                Int(floor(test_size * cls_ratio));
                                replace=false,

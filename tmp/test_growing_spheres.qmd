...
convergence_generator_conditions = CounterfactualExplanations.Convergence.GeneratorConditionsConvergence(max_iter=1000)
```
...
# Progress so far (21.02.2024)
1. `terminated()` function is working properly (disclaimer: `GrowingSpheresGenerator` always requires `GeneratorsConditionsConvergence` due to its peculiar nature).
2. The path of a generated counterfactual is NOT updated accordingly yet.
3. The dispatch on `update!()` function has to be moved to `generate_pertubations()`.

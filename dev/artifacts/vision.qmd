...
```{julia}
counterfactual_data = CounterfactualData(load_fashion_mnist()...)
X = counterfactual_data.X
y = counterfactual_data.output_encoder.y  
input_dim, n_obs = size(X)
```
...
```{julia}
test_data = CounterfactualData(load_fashion_mnist_test()...)
...
```{julia}
counterfactual_data = CounterfactualData(load_cifar_10()...)
X = counterfactual_data.X
y = counterfactual_data.output_encoder.y  
input_dim, n_obs = size(X)
```
...
```{julia}
test_data = CounterfactualData(load_cifar_10_test()...)
println("MLP test score (F1): $(model_evaluation(M, test_data)[1])")
println("Deep Ensemble test score (F1): $(model_evaluation(M_ens, test_data)[1])")
```

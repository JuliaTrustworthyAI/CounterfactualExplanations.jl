...
x_python = counterfactual_data.X
x_python = np.array(x_python)
x_python = torch.Tensor(x_python).T

y_python = counterfactual_data.y
...
# Returns Transpose{Float32, Matrix{Float32}}
...
y_chosen_factual_idx = rand(findall(predict_label(model_pytorch, counterfactual_data) .== factual))
...
```{julia}
CounterfactualExplanations.plot(M_flux, counterfactual_data)
```

### Plot
```{julia}
T = total_steps(counterfactual_flux)
X_path = reduce(hcat,path(counterfactual_flux))
ŷ = target_probs(counterfactual_flux, X_path)
p1 = CounterfactualExplanations.plot(counterfactual_flux)
```

```{julia}
T = total_steps(counterfactual_pytorch)
X_path = reduce(hcat,path(counterfactual_pytorch))
ŷ = target_probs(counterfactual_pytorch, X_path)
p1 = CounterfactualExplanations.plot(counterfactual_pytorch)
```

```{julia}
using Plots
anim = animate_path(counterfactual_pytorch)
gif(anim, "intro.gif", fps=2)
gif(anim, "$(pwd())/pytorch_test", fps=2)
```
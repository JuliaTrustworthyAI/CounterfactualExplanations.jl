...
counterfactuals = CounterfactualExplanations.counterfactual(counterfactual)
...
On the model side, we will use a simple multi-layer perceptron (MLP). Listing \ref{lst:mnist-setup} loads the data and the pre-trained MLP. It also loads a pre-trained Variational Auto-Encoder, which will be used by our counterfactual generator of choice for this task: REVISE. 
...
using Images
image_size = 300
function build_plot(x,ce;kwrgs...)
    ...
    plt = plot(p1,p2;size=(500,image_size),kwrgs...)
    return plt
end
plt = build_plot(x, ce; plot_title="Expressive VAE")
...
![Counterfactual explanations for MNIST using a Latent Space generator with an expressive generative model.](www/mnist_9to4_latent.png){#fig-mnist-4to9-latent width=13pc height=5pc}

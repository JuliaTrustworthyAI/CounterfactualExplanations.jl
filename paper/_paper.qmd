...
For our second example, we will look at image data. The MNIST dataset contains 60,000 training samples of handwritten digits in the form of 28x28 pixel grey-scale images \cite{lecun1998mnist}. Each image is associated with a label indicating the digit (0-9) that the image represents. The data makes for an interesting case study of Counterfactual Explanations because humans have a good idea of what realistic counterfactuals of digits look like. For example, if you were asked to pick up an eraser and turn the digit in @fig-mnist-factual into a four (4) you would know exactly what to do: just erase the top part. Schut et al. (2021) \cite{schut2021generating} leverage this idea to illustrate to the reader that their methodology produces realistic counterfactuals. In what follows we replicate some of their findings. You as the reader are therefore the perfect judge to evaluate the quality of the Counterfactual Explanations presented below. 
...
using Images
image_size = 300
p1 = plot(convert2image(MNIST, reshape(x,28,28)),axis=([],false), size=(image_size, image_size))
...
p2 = plot(
    ...
    axis=([],false), size=(image_size, image_size)
)
...
![Original image.](www/mnist_factual.png){#fig-mnist-factual width="20pc" height="30px"}

![REVISE with expressive VAE.](www/mnist_vae_strong.png){#fig-mnist-strong width="20pc" height="15pc"}

![REVISE with weak VAE.](www/mnist_vae_weak.png){#fig-mnist-weak width="20pc" height="15pc"}

Counterfactual explanations for MNIST using a Latent Space generator: turning a nine (9) into a four (4).
:::

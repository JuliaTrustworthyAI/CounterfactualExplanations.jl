...
Machine learning models like deep neural networks have become so complex, opaque and underspecified in the data that they are generally considered as black boxes. This lack of transparency exacerbates a number of other problems typically associated with these models: they tend to be instable (\cite{goodfellow2014explaining}), encode existing biases (\cite{buolamwini2018gender}) and learn representations that are surprising or even counter-intuitive from a human perspective (\cite{sturm2014simple}). Nonetheless, they often form the basis for data-driven decision-making systems. 
...
> “You cannot appeal to (algorithms). They do not listen. Nor do they bend.” \cite{o2016weapons}
...
The availability of open-source software for the purpose of explaining black-box models through counterfactuals is still limited. Most existing implmentations are specific to particular methodologies. They are also exclusively built in Python and for Python models. The only existing unifying software approach, for example, is tailored to models built in the two most popular Python libraries for deep learning. The Julia ecosystem has so far lacked an open-source implementation of counterfactual explanations. 
...
# Introducing: `CounterfactualExplanations.jl` {#sec-use}
...
The package currently offers native support for models built in the following libraries: [Flux](https://fluxml.ai/); [Torch for R](https://torch.mlverse.org/); [PyTorch](https://pytorch.org/).
In the following section we will present usage examples and explain how the package can be extended through custom generators and models. 

![Overview of package architecture. Modules are shown in red, structs in green and functions in blue.](www/pkg_architecture.png){#fig-arch width=20pc height=12pc}

# Basic Usage {#sec-start}
...
The API call for actually generating counterfactuals for our new model is the same as before. @fig-multi shows the resulting counterfactual path for a randomly chosen sample. In this case the contour shows the predicted probability that the input is in the target class ($t=`julia target`$). Generic search yields a valid, realistic and unambiguous counterfactual. 
...
## Adding Foreign Language Support {#sec-custom-foreign}

The Julia language offers unique support for programming language interoperability. For example, calling R or Python is made remarkably easy through auxiliary packages like `RCall.jl`, `PythonCall.jl` and `PyCall.jl`, respectively. This functionality has been leveraged to make `CounterfactualExplanations.jl` compatible with models that were developed in foreign programming languages. At the time of writing this feature is still somewhat experimental, but we hope that the following demonstration shows, firstly, how straight-forward it can be to add foreign language support and, secondly, how useful this may potentially be to the data science community at large. In particular, we will loosely reiterate here how we have added support for `torch` neural network trained in `R`.^[The corresponding example involving `PyTorch` is analogous and therefore not included here. You may find it here: [https://www.paltmeyer.com/CounterfactualExplanations.jl/dev/tutorials/interop/](https://www.paltmeyer.com/CounterfactualExplanations.jl/dev/tutorials/interop/)] 

### Explaining a Neural Network Trained in R
...
\begin{lstlisting}[language=Julia, escapechar=@, numbers=left, label={lst:generate}, caption={.}]
# Step 1)
struct MyTorchModel <: Models.AbstractDifferentiableModel
    nn::Any
end

# Step 2)
function logits(M::MyTorchModel, X::AbstractArray)
  nn = M.nn
  y = rcopy(R"as_array($nn(torch_tensor(t($X))))")
  y = isa(y, AbstractArray) ? y : [y]
  return y'
end
function probs(M::MyTorchModel, X::AbstractArray)
  return σ.(logits(M, X))
end
M = MyTorchModel(R"model")
\end{lstlisting}
...
# A Worked Example - MNIST {#sec-emp}
...
# MLP:
# Step 1)
struct MyFluxModel <: Models.AbstractFittedModel
    model::Any
end
# Step 2)
logits(M::MyFluxModel, X::AbstractArray) = M.model(X)
...
# Discussiong and Outlook {#sec-outlook}

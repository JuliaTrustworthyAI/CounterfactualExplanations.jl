...
To address the issues outlined above, we can make leverage the ideas underlying some of the more advanced counterfactual generators introduced above. In particular, we will now show how easy it is to compose custom generators that blend different ideas through user-friendly macros.
...
# Counterfactual search:
generator = GenericGenerator(opt=Flux.Descent(0.25))
ce = generate_counterfactual(
    x, target, counterfactual_data, M, generator)
```
...
plt = plot(plt, size=(500, 200), legend=:topright, xlabel="Income", ylabel="Age", margin=5mm, ylims=(-2.5,2.0))
display(plt)

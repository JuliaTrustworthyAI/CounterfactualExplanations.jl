...
yields good results in this setting.
...
prioritized it in the development of our package. While there is no free
lunch and \(M\in\mathcal{\widetilde{M}}\) may seem like a hard
constraint, recent advances in probabilistic machine learning have shown
that the computational cost involved in Bayesian model averaging is
lower than we may have thought (\cite{gal2016dropout},
...
{\centering \includegraphics[width=3.33333in,height=2.5in]{www/models_uml.png}
...
{\centering \includegraphics[width=3.33333in,height=3.33333in]{www/generators_uml.png}
...
community efforts will facilitate our long-term goals. Only two steps
are necessary to make any supervised learning model compatible with our
package\footnote{In order for the model to be compatible with the
  gradient-based default generators presented in Section~\ref{sec-start}
  gradient access is also necessary, but any model can also be
  complemented with a custom generator.}:
...
\cite{joshi2019towards} and ROAR \cite{upadhyay2021towards}. Through its
composable nature, our package may also allow for combining different
approaches.

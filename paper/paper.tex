...
factual \(x\) to the counterfactual \(\tilde{x}\). To
...
predictive entropy:
\(\arg \min _{\tilde{x}} \ell(M(\tilde{x}),t) \in \arg \min _{\tilde{x}} h(\tilde{x})\).
...
\texttt{confidence} in the final prediction \(M(\tilde{x})=t\).
...
\(\nabla_{\tilde{x}}\ell(M(\tilde{x},t))\) in this case is

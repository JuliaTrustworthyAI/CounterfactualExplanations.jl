...
\section{Laguage interoperability}\label{laguage-interoperability}}
...
\subsection{\texorpdfstring{Explaining a model trained in
...
function logits(M::TorchNetwork, X::AbstractArray)
  nn = M.nn
  y_hat = rcopy(R"as_array($nn(torch_tensor(t($X))))")
  y_hat = isa(y_hat, AbstractArray) ? y_hat : [y_hat]
  return y_hat
end
probs(M::TorchNetwork, X::AbstractArray)= (logits(M, X))
...
\texttt{∂ℓ(generator::GenericGenerator,\ x\_factual,\ 𝑴,\ t)}. In
particular, we will extend the function by a method that is specific to
the \texttt{TorchNetwork} type we defined above. The code below
implements this: our new method \texttt{∂ℓ} calls R in order to use
\texttt{torch}'s autodifferentiation functionality for computing the
gradient.
...
# Countefactual loss:
function (generator::GenericGenerator, x_factual, M::TorchNetwork, t) 
  nn = M.nn
  R"""
  x <- torch_tensor($x_factual, requires_grad=TRUE)
  output <- $nn(x)

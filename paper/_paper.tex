...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{using CounterfactualExplanations, Plots, PlotThemes, GraphRecipes}
\NormalTok{theme(:wong)}
\NormalTok{default(size=(500, 375))}
\NormalTok{include("paper/utils.jl")}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plot(code, method=:tree, fontsize=8, nodeshape=:rect, axis\_buffer=0.3)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p = plot(CounterfactualExplanations.Models.FittedModel, method=:tree, fontsize=8, nodeshape=:rect, axis\_buffer=0.3)}
\NormalTok{savefig(p, "paper/www/models.png")}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p = plot(CounterfactualExplanations.Generator, method=:tree, fontsize=8, nodeshape=:rect, axis\_buffer=0.3)}
\NormalTok{savefig(p, "paper/www/generators.png")}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{\# Data:}
\NormalTok{using CounterfactualExplanations, Random}
\NormalTok{Random.seed!(1234);}
\NormalTok{N = 100 \# number of data points}
\NormalTok{using CounterfactualExplanations.Data}
\NormalTok{x, y = toy\_data\_linear(N) }

\NormalTok{\# Model:}
\NormalTok{using CounterfactualExplanations.Models }
\NormalTok{w = [1.0 1.0]\# true coefficients}
\NormalTok{b = 0}
\NormalTok{M = LogisticModel(w, [b])}

\NormalTok{\# Setup:}
\NormalTok{x\_factual = x[rand(1:length(x))]}
\NormalTok{y\_factual = round(probs(M, x\_factual)[1])}
\NormalTok{target = ifelse(y\_factual==1.0,0.0,1.0) }
\NormalTok{confidence = 0.75 }

\NormalTok{\# Counterfactual search:}
\NormalTok{generator = GenericGenerator(}
\NormalTok{    0.1,0.1,1e{-}5,:logitbinarycrossentropy,nothing)}
\NormalTok{counterfactual = generate\_counterfactual(}
\NormalTok{    generator, x\_factual, M, target, confidence)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{X = hcat(x...)}
\NormalTok{plt = plot\_contour(X\textquotesingle{},y,M)}
\NormalTok{[scatter!(plt, [x[1]], [x[2]], ms=7.5, color=Int(y\_factual), label="") for x in counterfactual.path]}
\NormalTok{savefig(plt, "paper/www/ce\_binary.png")}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{w = [100.0 100.0]\# true coefficients}
\NormalTok{b = 0}
\NormalTok{M = LogisticModel(w, [b])}
\NormalTok{plt = plot\_contour(X\textquotesingle{},y,M)}
\NormalTok{scatter!(plt, [4], [{-}3], color="red", ms=10, label="Unrealistic CE")}
\NormalTok{scatter!(plt, [1e{-}5], [1e{-}5], color="purple", ms=10, label="Ambiguous CE")}
\NormalTok{savefig(plt, "paper/www/binary\_wrong.png")}
\end{Highlighting}
\end{Shaded}
...
expansion of the logistic binary cross-entropy function evaluated at the
...
\begin{Highlighting}[]
\NormalTok{\# Model:}
\NormalTok{using LinearAlgebra}
\NormalTok{I = UniformScaling(1)}
\NormalTok{cov = Symmetric(reshape(randn(9),3,3).*0.01 + I) }
\NormalTok{w = [1 1]}
\NormalTok{params = hcat(b, w)}
\NormalTok{M = BayesianLogisticModel(params, cov)}

\NormalTok{\# Counterfactual search:}
\NormalTok{generator = GreedyGenerator(}
\NormalTok{    0.25,20,:logitbinarycrossentropy,nothing)}
\NormalTok{counterfactual = generate\_counterfactual(}
\NormalTok{    generator, x\_factual, M, target, confidence)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{plt = plot\_contour(X\textquotesingle{},y,M)}
\NormalTok{[scatter!(plt, [x[1]], [x[2]], ms=7.5, color=Int(y\_factual), label="") for x in counterfactual.path]}
\NormalTok{savefig(plt, "paper/www/ce\_binary\_laplace.png")}
\end{Highlighting}
\end{Shaded}

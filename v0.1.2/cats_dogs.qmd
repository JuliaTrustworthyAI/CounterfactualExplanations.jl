...
artifact_dir = artifact"cats_dogs_images"
img_path = joinpath(artifact_dir, "cats_dogs_images")
if !isdir(www_path)
    mkdir(www_path)
end
[cp(joinpath(img_path, file), joinpath(www_path, file), force=true) for file in readdir(img_path)]
```
...
save(joinpath(www_path,"predictive.png"),img)
```

...
We can implement this metric in Julia as follows:
...
for λ₂ ∈ Λ₂  
    ...
    counterfactuals = vcat(
      counterfactuals...,
      generate_counterfactual(x, target, counterfactual_data, M, generator; γ=γ, num_counterfactuals=n_cf)
    )
end
```
...
```{julia}
theme(:wong)
T = maximum(map(x-> total_steps(x),counterfactuals)) - 1
p1 = plot_contour(X',ys,M;colorbar=false)
anim = @animate for t ∈ 1:T
    plts = []
    for i ∈ 1:length(Λ₂)
        λ₂ = Λ₂[i]
        counterfactual = counterfactuals[i]  
        plt = plot_contour(X',ys,M;colorbar=false, title="λ₂=$λ₂")
        idx = minimum([total_steps(counterfactual),t])
        x1 = vec(mapslices(X -> X[1], path(counterfactual)[idx], dims=(1,2)))
        x2 = vec(mapslices(X -> X[2], path(counterfactual)[idx], dims=(1,2)))
...
gif(anim, joinpath(www_path, "dice.gif"), fps=64)
```

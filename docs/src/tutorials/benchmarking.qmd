...
For convenience, the `DataFrame` containing the evaluation can be returned by simply calling the `Benchmark` object:

```{julia}
bmk()
```

To return the aggregated evaluation measures across `id` (the default behaviour of `evaluate`), simply do:
...
We suspect that this is the most common workflow for practitioners who are interested in benchmarking counterfactual explanations for the pre-trained machine learning models. Let's go through this workflow using a simple example. We firstly train some models and store them in a dictionary:

...
# Countefactual loss:
function ∂ℓ(generator::GenericGenerator, x̃, 𝑴::TorchNetwork, t) 
  nn = 𝑴.nn
  R"""
  x <- torch_tensor($x̃, requires_grad=TRUE)
  output <- $nn(x)
...
# [scatter!(p1, [counterfactual.path[t][1]], [counterfactual.path[t][2]], ms=5, color=Int(y), label="") for t in 1:T]
# p1
anim = @animate for t in 1:T
    scatter!(p1, [counterfactual.path[t][1]], [counterfactual.path[t][2]], ms=5, color=Int(y), label="")
...
# Countefactual loss:
function ∂ℓ(generator::GenericGenerator, x̃, 𝑴::PyTorchNetwork, t) 
  nn = 𝑴.nn
  x = reshape(x̃, 1, length(x̃))
  py"""
  x = torch.Tensor($x)

...
Among models from the MLJ library, only importing the `EvoTreeClassifier` is supported thus far. Generating counterfactuals is not supported for any MLJ model yet, but integration with the Feature Tweak generator will be provided soon. For that reason, a short overview of setting up MLJ models will be provided.
...
Using the usual procedure for defining models, we can call the following method:

...
Below we impose that the second feature is immutable. The resulting counterfactual path is shown in the chart below. Since only the first feature can be perturbed, the sample can only move along the horizontal axis.
...
#| fig-cap: Counterfactual path with immutable feature.
plot(ce)
```
...
counterfactual_data.domain = [(-Inf,0.1) for var in counterfactual_data.features_continuous]
```

...
Numerous evaluation measures for counterfactual explanations have been proposed. 

### Single Measure, Single Counterfactual

One of the most important measures is [`validity`](@ref), which simply determines whether or not a counterfactual explanation $x^{\prime}$ is valid in the sense that it yields the target prediction: $M(x^{\prime})=t$. We can evaluate the validity of a single counterfactual explanation `ce` using the [`Benchmark.evaluate`](@ref) function as follows:
...
```{julia}
using CounterfactualExplanations.Benchmark: distance_measures
distance_measures
```

...
Let $t\in\{0,1\}$ denote the target label, $M$ the model (classifier) and x̃ ∈ ℝᴰ the vector of counterfactual features. In order to generate recourse the `GenericGenerator` optimizes the following objective function through steepest descent

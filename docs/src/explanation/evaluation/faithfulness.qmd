...
p2 = plot(img, title="Generic: p(y=$target)=$(round(target_probs(ce)[1], digits=2))", axis=([], false))
...
println("Plausibility: $(round(plaus, digits=5))")
img = convert2image(MNIST, reshape(ce.x′, 28, 28))
p3 = plot(img, title="ECCo: p(y=$target)=$(round(target_probs(ce)[1], digits=2))", axis=([], false))

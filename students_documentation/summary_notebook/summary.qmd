...
This leaves us with 0 as the target class. We can now generate the counterfactual as usual:

```{julia}
target = 0
moons_counterfactual = generate_counterfactual(x, target, counterfactual_data, decision_tree, generator)
```

Let's verify that the counterfactual is indeed in the target class:

```{julia}
Models.predict_label(decision_tree, moons_counterfactual.s′)
```

Finally, we can visualize the generated counterfactual and the decision boundaries of the decision tree:

```{julia}
CounterfactualExplanations.plot(moons_counterfactual)
```

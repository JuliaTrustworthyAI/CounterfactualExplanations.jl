...
CounterfactualExplanations.plot(blobs_counterfactual; title = "Counterfactuals on a PyTorch model")
```
...
```{julia}
CounterfactualExplanations.plot(tree_counterfactual; colorbar=false, title = "Feature Tweak counterfactuals on a Decision Tree")
```

```{julia}
CounterfactualExplanations.plot(forest_counterfactual; title = "Feature Tweak counterfactuals on a Random Forest")
```
...
```{julia}
y_chosen_factual_idx = rand(findall(predict_label(mlp_model, counterfactual_data) .== factual))
```
